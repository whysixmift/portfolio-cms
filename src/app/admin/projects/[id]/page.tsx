...
export default async function ProjectEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  let project = null;
  if (!isNew) {
    project = await db.project.findUnique({ where: { id: resolvedParams.id } });
    if (!project) notFound();
  }

  return (
    <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-2 duration-200">
      <Link href="/admin/projects" className="inline-flex items-center gap-2 font-mono text-xs uppercase tracking-widest text-zinc-500 hover:text-white transition-colors w-fit">
      ...
      <div className="border-b border-[#262626] pb-6">
        <h1 className="text-3xl font-semibold tracking-tight text-white mb-2 uppercase">
          {isNew ? "Create Project" : "Modify Configuration"}
        </h1>
        <p className="font-mono text-xs text-zinc-500 uppercase tracking-widest">
          // {isNew ? "Initialize new database entry" : `TARGET: ${project?.id}`}
        </p>
      </div>
